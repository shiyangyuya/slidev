 ...
 */
export function encodeToUrlSafeBase64(data) {
    // 将数据转换为UTF-8编码的字节数组
...
const ssoLogin = (redirect) => {
    const key = location.host.slice(0, location.host.indexOf("."));
    const tk = cookies.get("_xzkj_") || "";
    // sso登录地址
    const url = `https://${hostMap[key]}.scmttec.com/passport/sso.do?redirect=${window.encodeURIComponent(redirect)}&tk=${encodeToUrlSafeBase64(tk)}&from=${location.host}`;
    location.href = url;
};
